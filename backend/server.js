...
app.get('/api/health', (req, res) => {
    res.json({
        status: 'OK',
        message: 'Shawrap Inn API running',
...
const connectDB = async () => {
    try {
        ...
        if (!adminExists) {
            await User.create({
                name: 'Admin',
                email: process.env.ADMIN_EMAIL || 'admin@shawrapinn.com',
                password: process.env.ADMIN_PASSWORD || 'Admin@123',
                phone: '9999999999',
                role: 'admin',
            });
            console.log('✅ Admin user created → admin@shawrapinn.com / Admin@123');
        }
    } catch (error) {
...
const startServer = async () => {
    try {
        ...
        app.listen(PORT, () => {
            console.log(`🚀 Shawrap Inn API → http://localhost:${PORT}`);
